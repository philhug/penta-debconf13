CREATE OR REPLACE VIEW debconf.dc_when_do_they_get_here AS
  SELECT DISTINCT person.person_id, person.nickname,
  	 person.country, dc_person_type.description,
  ...
  WHERE conference_person.conference_id = 5 AND
  	conference_person_travel.arrival_date IS NOT NULL AND
	conference_person_travel.departure_date IS NOT NULL AND
	dc_conference_person.attend = true AND
	conference_person.reconfirmed = true
  ORDER BY person.nickname, person.country,
  	dc_person_type.description, person.first_name,
	person.last_name, conference_person_travel.arrival_date,
	conference_person_travel.departure_date, person.person_id;
